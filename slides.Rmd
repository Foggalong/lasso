...
<!-- Blocks that do set-up for the rest of the document-->
...
```r
full_model <- lm(full_rate~., data=loans_ids)
...
Starting with the least squares problem, $\min_{{\bf\beta}}(\operatorname{RSS})$, we add an additional constraint

\begin{equation}
  \min_{\beta} \left(\sum_{i=1}^n {\left( y^{(i)} - \beta_0 - \sum_{j=1}^p\beta_{j}x_{j}^{(i)} \right)}^2 \right)~\text{subject to}~\|\beta\|\leq s.
\end{equation}

on our vector of coefficients. Here $t$ is a parameter to be tuned.


--

By rewriting this in **Lagrange form**, we can work with the equivalent problem

\begin{equation}
  \min_{\beta} \left(\sum_{i=1}^n {\left( y^{(i)} - \beta_0 - \sum_{j=1}^p\beta_{j}x_{j}^{(i)} \right)}^2 + \lambda\|\beta\| \right).
\end{equation}

where $\lambda\geq0$ is now the tuning parameter.

---

### The Ridge

In **Ridge regression**, we apply the $\mathcal{l}_1$ norm, solving $\min_{\beta} (RSS + \lambda\sum_{i=1}^p\beta_j^2 )$.
...
model = glmnet(X, y, alpha = 0)  # use L1 norm
```
...
Note that $\beta_i>0$ for all $i$ and $\lambda$ here.

---

### The Lasso

With **Lasso regression** we apply the $\mathcal{l}_2$ norm, solving $\min_{\beta} (RSS + \lambda\sum_{i=1}^p|\beta_j^{\phantom{'}}| )$.

```{r lasso-regression, echo=TRUE, class.output="scroll-300", warning = FALSE, message = FALSE}
# glmnet requires matrix input, not formula
X <- as.matrix(loans_ids[, -1])  # first column is interest_rate
y <- as.matrix(loans_ids[, 1])
model = glmnet(X, y, alpha = 1)  # use L2 norm
```
...
y <- rnorm(n)
ridge_model <- glmnet(X, y, alpha=0)  # L1 norm
lasso_model <- glmnet(X, y, alpha=1)  # L2 norm
```

```{r, echo=FALSE, fig.height=4.5, fig.width=5.2, dev='svg'}
plot1 <- plot(ridge_model, xvar="lambda", main="Ridge Coefficients\n")
plot2 <- plot(lasso_model, xvar="lambda", main="Lasso Coefficients\n")
...
```{r}
summary(mtcars)  # for a slide we should never get to
```
