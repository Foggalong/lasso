...
library(gridExtra)   # easy image grids 
```

```{css scrolling-output, echo=FALSE}
.scroll-300 {
...
```{r scatter-plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=8, dev='svg', fig.alt="Scatter diagram for the classic 'mtcars' dataset, showing miles per gallon against horsepower. A linear regression of y = 30 - 0.068x is shown."}
...
```{r step-backwards, echo=TRUE, class.output="scroll-300"}
...
```{r step-forwards, echo=TRUE, class.output="scroll-300"}
...
```{r step-both, echo=TRUE, eval=FALSE, class.output="scroll-300"}
empty_model <- lm(interest_rate~1, data=na.omit(loans_ids))
...
<br><br><br>  <!-- Footnote support seems to be broken -->

_Note: we can show the above is the **Lagrange form** of constraining LS by some ${\|\beta\|} \leq t$._
...
```{r load-data-nona, echo=FALSE}
loans_ids <- na.omit(loans_ids)
```

```{r ridge-regression, echo=TRUE, class.output="scroll-300", warning = FALSE, message = FALSE}
...
```{r ridge-coefficient-plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, dev='svg'}
...
```{r lasso-regression, echo=TRUE, class.output="scroll-300", warning = FALSE, message = FALSE}
# glmnet requires matrix input, not formula
X <- as.matrix(loans_ids[, -1])  # first column is interest_rate
y <- as.matrix(loans_ids[, 1])
...
```{r lasso-coefficient-plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, dev='svg'}
plot(model, label = TRUE, xvar = "lambda")
```
...
```{r fig67, echo=FALSE, out.width="80%", fig.show="hold",  fig.align="center", fig.alt="Hello, world!", fig.cap="Contour plot of the error and constraint functions for the lasso and ridge respectively."}
...
```{r, echo=FALSE, fig.height=4.5, fig.width=5.2, dev='svg'}
plot0 <- plot(ridge_model, xvar="lambda", main="Ridge Coefficients\n")
plot1 <- plot(lasso_model, xvar="lambda", main="Lasso Coefficients\n")
...
```{r, echo=FALSE, fig.height=2.8, fig.width=3.4, dev='svg'}
plot.R <- plot(glmnet(X, y, alpha=0), xvar="lambda", main="Elastic (a = 0)\n")
plot.2 <- plot(glmnet(X, y, alpha=1e-3), xvar="lambda", main="Elastic (a = 0.001)\n")
plot.4 <- plot(glmnet(X, y, alpha=1e-2), xvar="lambda", main="Elastic (a = 0.01)\n")
plot.6 <- plot(glmnet(X, y, alpha=1e-1), xvar="lambda", main="Elastic (a = 0.1)\n")
plot.8 <- plot(glmnet(X, y, alpha=5e-1), xvar="lambda", main="Elastic (a = 0.5)\n")
plot.L <- plot(glmnet(X, y, alpha=1e-0), xvar="lambda", main="Elastic (a = 1)\n")
grid.arrange(plot.R, plot.2, plot.4, plot.6, plot.8, plot.L, ncol=3)
```
